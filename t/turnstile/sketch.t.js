...
function prove (async, assert) {
    ...
    */
    ...
    //
    // It may be the case that the caller should not proceded until the work is
    ...
    //
    // The next question becomes, what sort of error could be recoverable at thi
    // point, so that error handling is meaningful? Do we want to report the
    // errors that the work generates, or do we simply want to report that the
    // worker failed in some way? We can simply report some sort of write error,
    // and we can garuntee that it will always be an error of a single type,
    // that is, that we're not going to pass on the error thrown by the worker,
    // and in fact, at this point, we can garuntee that there will be an error
    // array, since this is not cadence.
    //
